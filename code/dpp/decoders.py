...
import dpp.tails as tails
import dpp.centers as centers
...
def RQS_EXP(general_config, left = 0, right = 0.95, bottom = 0, min_qtail = 0, max_qtail = 1, num_bins = 10, hypernet_hidden_sizes=[64], **kwargs):
	
    center_model = centers.RationalQuadraticSpline(left, right, bottom, min_qtail, max_qtail, num_bins)
    tail_model = tails.ExponentialTail(right)
    return dist.PieceWiseQuantileFunction(config = general_config, center_model = center_model, 
                                            tail_model = tail_model, 
                                            hypernet_hidden_sizes = hypernet_hidden_sizes, 
                                            train_widths = kwargs["train_widths"],
                                            log_and_scaling = kwargs["log_and_scaling"],
                                            scale_init = kwargs["scale_init"],
                                            calib_regularization = kwargs["calib_regularization"]
                                            )


def EXP(general_config, hypernet_hidden_sizes=[64], **kwargs):
    return dist.ExponentialQuantileFunction(config = general_config, hypernet_hidden_sizes = hypernet_hidden_sizes)
