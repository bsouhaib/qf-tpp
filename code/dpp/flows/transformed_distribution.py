...
class TransformedDistribution(BaseModule):
    ...
    def calibration_error(self, y, h, emb, params=None, N = 100):
        mat_ind = torch.empty(y.shape[0], y.shape[1], N)
        tau_all = torch.linspace(0, 1-1e-9, N)
        for i in range(N):
            mat_ind[:, :, i] = torch.exp(self.log_cdf(y, h=h, emb=emb)) <= tau_all[i]
        res = torch.abs(torch.mean(mat_ind, (0, 1)) - tau_all)
